// AppFence/DexGuard bypass v24-fixed - GC-SAFE version
...
//
// CRITICAL: All NativeCallbacks and Memory.alloc buffers MUST be global
// to prevent garbage collection. Inline callbacks in onEnter get GC'd → crash.
//
...
var mapsFiles = new Set();

// === GLOBAL REFERENCES (prevent GC) ===
var nanosleepFn = new NativeFunction(
    Module.findExportByName("libc.so", "nanosleep"), 'int', ['pointer', 'pointer']);
var sleepBuf = Memory.alloc(16);
sleepBuf.writeU64(0x7FFFFFFF);
sleepBuf.add(8).writeU64(0);

var killThreadReplacement = new NativeCallback(function(arg) {
    console.log("[+] Kill thread sleeping forever");
    nanosleepFn(sleepBuf, ptr(0));
    return ptr(0);
}, 'pointer', ['pointer']);
    ...
    onEnter: function(args) {
        ...
        if (mod && mod.name === LIB_NAME) {
            ...
            if (offset === KILL_THREAD_OFFSET) {
                console.log("[+] Blocking kill thread at +0x" + offset.toString(16));
                args[2] = killThreadReplacement; // Use GLOBAL ref, not inline callback
            }
        }
    }
});
...
Interceptor.attach(fgetsPtr, {
    onEnter: function(args) {
        ...
        this.stream = args[2];
        this.isMaps = mapsFiles.has(this.stream.toString());
    },
    onLeave: function(retval) {
...
console.log("[+] AppFence bypass loaded (GC-safe)");
