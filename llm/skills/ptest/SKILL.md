...
- **Internal engagements: always request the zone file first**: For internal pentests where the operator has authorized access, requesting the DNS zone file (Cloudflare export, Route53 dump, Azure DNS list) before spending hours on brute-force is mandatory. A 5-minute export gives you 100% coverage that days of brute-forcing cannot achieve. This is now a Phase 1 mandatory step for internal engagements. If the zone file is unavailable, document the gap — but always ask.
