...
- **nuclei timeout**: nuclei with all templates against targets behind Cloudflare/WAF is extremely slow. Use `-timeout 5 -retries 1 -c 25` flags. Run against non-WAF targets first (direct nginx/GCP hosts). Manual testing is often more productive than waiting for nuclei on WAF-protected targets. **Hetzner-hosted targets** also aggressively rate-limit nuclei — even with reduced concurrency, scans hang for 8+ minutes with zero output. For rate-limited hosts: run nuclei in background (`terminal background=true`), set a 5-minute mental deadline, and proceed with manual testing in parallel. If nuclei produces nothing after 5 minutes, kill it and document the gap. Don't block Phase 5 waiting for nuclei on slow targets. Also: Hetzner-hosted targets aggressively rate-limit nuclei even without a WAF — scans hang indefinitely. For rate-limited hosts, skip nuclei entirely and use targeted manual testing. Document the gap in the report.
