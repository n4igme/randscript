---
name: hunt-xss
description: Hunting skill for xss vulnerabilities. Built from 174 public bug bounty reports. Includes DOM XSS sinks (innerHTML, document.write, eval, setTimeout, location.href), stored XSS patterns, CSP bypass techniques (Angular template injection, mXSS), XSS chain escalation (CSRF token theft, service worker persistence, fake login ATO). Use when hunting xss on any target.
...
## XSS Chain Escalation (escalate to High/Critical)
