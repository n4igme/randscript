---
name: ref-ptest-integration
description: "Handoff from black-box penetration testing to white-box code review. Use when combining pentest findings with source analysis."
---

# ptest ↔ Source Code Review Integration Guide

Bidirectional workflow for combining external penetration testing (ptest) with source code review (sc1–sc5 pipeline) on the same target.
...
- sc3-vuln-scan found code vulnerabilities and you want to confirm exploitability externally
- Internal pentest engagement with full access

---

## Direction 1: ptest Findings → sc3 Scanner Focus
...
## Direction 2: sc4 Validated Findings → ptest Exploitation

When code review finds a vulnerability, use it to craft targeted exploits.

### Missing @PreAuthorize on Endpoint

**code-review finding:** Controller method lacks authorization annotation
...
**code-review finding:** User input reaches SpEL expression evaluation
...
**code-review finding:** Jackson `ObjectMapper` configured with `enableDefaultTyping()`
...
**code-review finding:** API key or secret in source code or config file
...
**code-review finding:** String concatenation in SQL query
...
**code-review finding:** User input used in URL construction
...
**code-review finding:** State-changing operation without synchronization or optimistic locking
...
**code-review finding:** CSRF protection disabled or endpoint excluded
...
### Scenario A: ptest First, Then code-review
...
4. **Run targeted code-review review** on affected components:
...
→ code-review: Check OrderController.java → missing @PreAuthorize
→ code-review: grep all controllers → 12 more endpoints with same issue
→ Report: 13 IDOR vulnerabilities, 1 externally confirmed, 12 from code review
```

---

### Scenario B: code-review First, Then ptest

Best when: You have source code access before network access (e.g., pre-deployment review).

1. **Complete code-review steps 1-4** — find code vulnerabilities
...
**Example (Spring Boot + Keycloak):**
```
sc3-vuln-scan found: SQL injection in UserRepository.java native query
...
2. **Simultaneously start code-review Step 1-2** (recon + threat model):
   - Map architecture, identify high-risk components
3. **Share intelligence bidirectionally:**
   - ptest discovers endpoints → code-review knows what code to focus on
   - code-review finds auth gaps → ptest knows what to exploit
   - ptest finds tech versions → code-review checks for known vulnerable patterns
4. **ptest Phase 5-6 informed by code-review findings:**
   - Exploit code-confirmed vulnerabilities
   - Skip false positives identified by code review
5. **code-review Step 3 scanners prioritized by ptest attack surface:**
...
```
Day 1: ptest recon + code-review architecture review
Day 2: ptest enumeration + code-review threat model
Day 3: Intelligence sharing session — align priorities
Day 4: ptest exploitation (guided by code-review) + code-review deep scan (guided by ptest surface)
...
Map ptest phases to code-review scanners for efficient parallel work.

### Phase 3: Actuator/Admin Interfaces Found

**Priority code-review scanners:** `misconfig`, `data-exposure`
...
**Priority code-review scanners:** `misconfig` (CORS section)
...
**Priority code-review scanners:** `authn-session`, `misconfig`
...
**Priority code-review scanners:** `access-control`
...
**Priority code-review scanners:** `injection`
...
**Priority code-review scanners:** `ssrf`
...
**Priority code-review scanners:** `data-exposure`, `crypto`
...
### From ptest context, pivot to code-review:

```
"I found [vulnerability] on [endpoint]. Run code-review [scanner] focused on the code handling that endpoint."
```

### From code-review context, pivot to ptest:
...
```
"Validate code-review finding [X] externally: the code at [file:line] shows [vulnerability]. 
...
| Component | ptest Surface | code-review Location |
|---|---|---|
...
**Key insight:** React client-side checks are never security controls. If code-review finds a route guard without corresponding server-side authorization, that's a ptest target — bypass the client and hit the API directly.
