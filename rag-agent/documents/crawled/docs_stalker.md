...
[ ](https://github.com/frida/frida-website/edit/main/_i18n/en/_docs/stalker.md)
# Stalker
## Introduction
Stalker is Frida’s code tracing engine. It allows threads to be followed, capturing every function, every block, even every instruction which is executed. A very good overview of the Stalker engine is provided [here](https://medium.com/@oleavr/anatomy-of-a-code-tracer-b081aadb0df8) and we recommend that you read it carefully first. Obviously, the implementation is somewhat architecture specific, although there is much in common between them. Stalker currently supports the AArch64 architecture commonly found on mobile phones and tablets running Android or iOS, as well as the Intel 64 and IA-32 architectures commonly found on desktops and laptops. This page intends to take things to the next level of detail, it dissects the ARM64 implementation of Stalker and explains in more detail exactly how it works. It is hoped that this may help future efforts to port Stalker to other hardware architectures.
## Disclaimer
Whilst this article will cover a lot of the details of the inner workings of Stalker, it won’t cover back-patching in real detail. It is intended as a starting point to help others understand the technology and Stalker is fiendishly complicated enough without this! To be fair though, this complexity isn’t there without reason, it is there to minimize the overhead of what is an inherently expensive operation. Lastly, while this article will cover the key concepts of the implementation and will extract some critical parts of the implementation for a line-by-line analysis, there will be some last details of the implementation left for the reader to discover by reading the [source code](https://github.com/frida/frida-gum/blob/master/gum/backend-arm64/gumstalker-arm64.c). However, it is hoped it will prove to be a very useful head-start.
## Table of contents
  1. [Introduction](https://frida.re/docs/stalker/#introduction)
  2. [Disclaimer](https://frida.re/docs/stalker/#disclaimer)
  3. [Use Cases](https://frida.re/docs/stalker/#use-cases)
  4. [Following](https://frida.re/docs/stalker/#following)
    1. [gum_stalker_follow_me](https://frida.re/docs/stalker/#gum_stalker_follow_me)
    2. [gum_stalker_follow](https://frida.re/docs/stalker/#gum_stalker_follow)
  5. [Basic Operation](https://frida.re/docs/stalker/#basic-operation)
  6. [Options](https://frida.re/docs/stalker/#options)
  7. [Terminology](https://frida.re/docs/stalker/#terminology)
    1. [Probes](https://frida.re/docs/stalker/#probes)
    2. [Trust Threshold](https://frida.re/docs/stalker/#trust-threshold)
    3. [Excluded Ranges](https://frida.re/docs/stalker/#excluded-ranges)
    4. [Freeze/Thaw](https://frida.re/docs/stalker/#freezethaw)
    5. [Call Instructions](https://frida.re/docs/stalker/#call-instructions)
    6. [Frames](https://frida.re/docs/stalker/#frames)
    7. [Transformer](https://frida.re/docs/stalker/#transformer)
    8. [Callouts](https://frida.re/docs/stalker/#callouts)
    9. [EOB/EOI](https://frida.re/docs/stalker/#eobeoi)
    10. [Prologues/Epilogues](https://frida.re/docs/stalker/#prologuesepilogues)
    11. [Counters](https://frida.re/docs/stalker/#counters)
  8. [Slabs](https://frida.re/docs/stalker/#slabs)
  9. [Blocks](https://frida.re/docs/stalker/#blocks)
  10. [Instrumenting Blocks](https://frida.re/docs/stalker/#instrumenting-blocks)
  11. [Helpers](https://frida.re/docs/stalker/#helpers)
    1. [last_stack_push](https://frida.re/docs/stalker/#last_stack_push)
    2. [last_stack_pop_and_go](https://frida.re/docs/stalker/#last_stack_pop_and_go)
  12. [Context](https://frida.re/docs/stalker/#context)
  13. [Context Helpers](https://frida.re/docs/stalker/#context-helpers)
  14. [Reading/Writing Context](https://frida.re/docs/stalker/#readingwriting-context)
  15. [Control flow](https://frida.re/docs/stalker/#control-flow)
  16. [Gates](https://frida.re/docs/stalker/#gates)
  17. [Virtualize functions](https://frida.re/docs/stalker/#virtualize-functions)
    1. [gum_exec_block_virtualize_branch_insn](https://frida.re/docs/stalker/#gum_exec_block_virtualize_branch_insn)
    2. [gum_exec_block_virtualize_ret_insn](https://frida.re/docs/stalker/#gum_exec_block_virtualize_ret_insn)
  18. [Emitting events](https://frida.re/docs/stalker/#emitting-events)
  19. [Unfollow and tidy up](https://frida.re/docs/stalker/#unfollow-and-tidy-up)
  20. [Miscellaneous](https://frida.re/docs/stalker/#miscellaneous)
    1. [Exclusive Store](https://frida.re/docs/stalker/#exclusive-store)
    2. [Exhausted Blocks](https://frida.re/docs/stalker/#exhausted-blocks)
    3. [Syscall Virtualization](https://frida.re/docs/stalker/#syscall-virtualization)
    4. [Pointer Authentication](https://frida.re/docs/stalker/#pointer-authentication)
...
If this instruction is copied to a different location in memory and executed, then because the address of the label is calculated by adding an offset to the current instruction pointer, then the value would be different. Fortunately, Gum has a [Relocator](https://github.com/frida/frida-gum/blob/master/gum/arch-arm64/gumarm64relocator.c) for just this purpose which is capable of modifying the instruction given its new location so that the correct address is calculated.
...
### EOB/EOI
Recall that the [Relocator](https://github.com/frida/frida-gum/blob/master/gum/arch-arm64/gumarm64relocator.c) is heavily involved in generating the instrumented code. It has two important properties which control its state.
End of Block (EOB) indicates that the end of a block has been reached. This occurs when we encounter _any_ branch instruction. A branch, a call, or a return instruction.
...
Note however, that the code necessary to write out the necessary CPU registers (the prologue) in either case is quite long (tens of instructions). And the code to restore them afterwards (the epilogue) is similar in length. We don’t want to write these at the beginning and end of every block we instrument. Therefore we write these (in the same way we write the instrumented blocks) into a common memory location and simply emit call instructions at the beginning and end of each instrumented block to call these functions. These common memory locations are referred to as _helpers_. The following functions create these prologues and epilogues.
```
...
Let’s now look in more detail at the _virtualizing_ we have for replacing the branch instruction we find at the end of each block. We have four of these functions:
...
[Back](https://frida.re/docs/hacking/)
[Next](https://frida.re/docs/presentations/)
##### Sponsored by:
